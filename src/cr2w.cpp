...
// The element type of an "array:X" / "static:N,X" type name.
...
redfs_status cr2w_walk_array(const redfs_cr2w* f, const redfs_value* array, redfs_elem_fn fn,
    ...
    const uint32_t width = fixed_width(elem);

    for (uint32_t i = 0; i < count; ++i) {
        const uint8_t* end;
        if (width) {
            end = p + width;
            if (end > limit) return REDFS_E_CORRUPT;
        } else {
            end = struct_end(p, limit);
            if (!end) return REDFS_E_CORRUPT;
        }
...
void cr2w_decode(const redfs_cr2w* f, const char* type, const uint8_t* data, uint32_t size,
    ...
    if (eq("Bool") && size >= 1) {
        ...
        const uint32_t chars = static_cast<uint32_t>(prefix < 0 ? -prefix : prefix);
        auto* s = new std::string();
        if (prefix > 0) {  // UTF-16
            const uint32_t bytes = chars * 2;
            if (p + bytes <= data + size) {
                s->reserve(chars);
                for (uint32_t i = 0; i < chars; ++i) {
                    const uint16_t wc = rd16(p + i * 2);
                    s->push_back(wc < 0x80 ? static_cast<char>(wc) : '?');
                }
            }
        } else if (p + chars <= data + size) {
            s->assign(reinterpret_cast<const char*>(p), chars);
        }
        const_cast<redfs_cr2w*>(f)->owned_strings.emplace_back(s);
        ...
        out->as.buffer = static_cast<uint32_t>(rd16(data)) - 1u;
...
redfs_status cr2w_parse(const void* data, uint64_t size, redfs_cr2w* out) {
    ...
    if (strings.count == 0 || b[strings.offset + strings.count - 1] != '\0')
